...
import type { AppCandidate } from "./speculosProxy";
import {
...
import {
  createSpeculosDevice as createSpeculosDeviceProxy,
  releaseSpeculosDevice as releaseSpeculosDeviceProxy,
  findAppCandidate as findAppCandidateProxy,
} from "./speculosProxy";
...
export async function runWithAppSpec<T extends Transaction>(
  ...
  const appCandidate = await findAppCandidateProxy(appQuery);
  if (!appCandidate) {
  ...
  try {
    device = await createSpeculosDeviceProxy(deviceParams);
    ...
    for (let j = 0; j < totalTries; j++) {
      for (let i = 0; i < length; i++) {
        ...
        log("bot/result", formatReportForConsole(report));
        ...
        if (
          ...
          await releaseSpeculosDeviceProxy(device.id);
          device = await createSpeculosDeviceProxy(deviceParams);
        }
      }
      mutationsCount = {};
    ...
    if (device) await releaseSpeculosDeviceProxy(device.id);
  }
